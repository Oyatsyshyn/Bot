...
def get_last_massages_webex(roomId_To_Get_Messages, count_last_massage):
    ...
    if len(json_data["items"]) == 0:
        Exception("There are no messages in the room.")
...
# функція для надсилання Post запиту створення і відправлення зображення та тексту з можливістю відповідати на повідомлення з командою
# в Webex при введених параметрах: Id кімнати, текст повідомлення, URL-адреса зображення, Id батківського повідомлення
def post_send_photo_and_text_webex(roomIdToGetMessages, responseText, responseFileURL, parentId_To_Get_Messages):
    # записуємо в змінну токен для Webex
    accessToken = "Bearer " + config.hard_Token_webex
    # заголовок HTTP Webex Teams, включають заголовок Content-Type для даних POST JSON
    HTTPHeaders = {
